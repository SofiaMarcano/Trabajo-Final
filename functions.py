        ...
        if paciente:
            ...
            if diagnosticos:
                ...
                for diag in diagnosticos:
                    print(f"ID Diagnóstico: {diag[0]}\nTipo de Imagen: {diag[2]}\nParte del cuerpo: {diag[3]}\nCondicion sugerida: {diag[4]}\nProbabilidad: {diag[5]}\nNotas IA: {diag[6]}\nComentarios del medico: {diag[7]}\nFecha diagnostico: {diag[8]}\nEstado: {diag[9]}")
            else:
    ...
    try:
        ...
        if not result:
            ...
            if con == 2:
                ...
                rep_colection.delete_one({"paciente_id": paciente_id})
...
def create_diag():
    try:
        ...
        fecha_diagnostico = rev_fecha(date)
        ...
        try:
            ...
            if reporte:
                # Si el reporte existe, agregar el nuevo diagnóstico a la lista de diagnósticos
                rep_colection.update_one(
                    {"paciente_id": paciente_id},
                    {"$push": {"diagnosticos": diag_doc}}
                )
                print("Diagnóstico añadido a la lista de diagnósticos en MongoDB.")
            else:
                print(f"No se encontró un reporte con el paciente_id: {paciente_id}. Se procede a crear un nuevo reporte con el diagnóstico.")
                try:
                    # Obtener el último ID de reporte
                    last_reporte = rep_colection.find().sort("id_reporte", -1).limit(1)
                    last_reporte = list(last_reporte)
                    ...
                    # Opcional: añadir notas adicionales
                    ...
                    reporte = {
                        ...
                        "diagnosticos": [diag_doc],  # Diagnóstico como lista
        ...
        if diagnostico:
            ...
            while True:
                ...
                if opcion == 1:
                    ...
                    if tipo_imagen != diagnostico[1]:  # Comparar si es diferente al valor actual
                        cursor.execute("""
                            UPDATE diagnosticos
                            SET tipo_imagen = %s
                            WHERE diagnostico_id = %s
            ...
            # Actualizar en MongoDB para todos los casos
            db = con_mongodb()
            rep_colection = db["reportes"]
            paciente_id = diagnostico[1]  # Asumiendo que el paciente_id está en la primera columna del diagnóstico

            # Buscar el reporte correspondiente al paciente
            reporte = rep_colection.find_one({"paciente_id": paciente_id})
            if reporte:
                ...
                for diag in diagnosticos:
                    if diag["diagnostico_id"] == diagnostico_id:
                        ...
                        if opcion == 1:
                            ...
                            diag["analisis_IA"]["condicion_sugerida"] = condicion_sugerida
                        elif opcion == 3:
                            diag["analisis_IA"]["probabilidad_%"] = probabilidad
                        elif opcion == 4:
                            diag["analisis_IA"]["notas_ia"] = notas_ia
                        elif opcion == 5:
                            diag["estado"] = estado
                ...
                print(f"No se encontró un reporte con el paciente_id: {paciente_id} en MongoDB.")
...
def del_diag():
    ...
    try:
        ...
        # Eliminar diagnóstico en MongoDB
        db = con_mongodb()
        rep_colection = db["reportes"]
        if rep_colection is not None:
            resultado = rep_colection.update_one(
                {"diagnosticos.diagnostico_id": diagnostico_id},  # Buscar el reporte que contiene el diagnóstico
                {"$pull": {"diagnosticos": {"diagnostico_id": diagnostico_id}}}  # Eliminar diagnóstico de la lista
            )
