    ...
    if opc == 1:
        try:
            while True:
                ...
                fecha_diag = rev_fecha("Ingrese la fecha del diagnóstico (YYYY-MM-DD): ")
                while True:
                    est = rev_num("Ingrese el estado del diagnóstico (1. Pendiente, 2. Confirmado, 3. Descartado): ")
                    if est == 1:
                        estado = "Pendiente"
                ...
                cursor.execute(query, (
                    id_diagnostico, paciente_id, tipo_imagen, parte_cuerpo, condicion_sugerida,
                    probabilidad, notas_ia, diag_doc["comentarios_medico"], fecha_diag, estado
                ))
        ...
        print("No se añadió ningún diagnóstico.")
    reporte_doc = {
...
def act_reporte():
    db = con_mongodb()
    rep_colection = db["reportes"]
    id_reporte = rev_num("Ingrese el ID del reporte a actualizar: ")
    reporte = rep_colection.find_one({"reporte_id": id_reporte})
    if not reporte:
        ...
        if opc == 1:
            ...
            if not diagnostico:
                ...
                rep_colection.update_one(
                    {"reporte_id": id_reporte, "diagnosticos.id_diagnostico": diagnostico_id},
                    {"$set": {
...
def add_nota_tec(username):
    db = con_mongodb()
    rep_colection = db["reportes"]
    id_reporte = rev_num("Ingrese el ID del reporte al que desea añadir notas técnicas: ")
    reporte = rep_colection.find_one({"reporte_id": id_reporte})
    if not reporte:
        print("Reporte no encontrado.")
    else:
